...
def captioner():
    ...
    doc = doclod(fileimgtxt)
    descriptions = takedesc(doc)
    parsedesc(descriptions)
    vocabulary = bagofwords(descriptions)
    savedesc(descriptions, 'app/descriptions.txt')
    filetraintxt = "app/trainImages.txt"
    train = loaddesc(filetraintxt)
...
#save descs to file
def savedesc(descriptions, filename):
    lines = list()
    for key, desc_list in descriptions.items():
